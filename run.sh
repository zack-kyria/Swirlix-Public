...
if [ "$SHOW_TAGS" == true ] ; then
  git fetch --tags
  git tag --list 'swirlix-v*' | sort -rV
  echo ""
...
if [ "$TESTING" = "true" ]; then
  # Get the latest Testing Git tag for the "swirlix-" prefix
  latest_swirlix_tag=$(git tag --list| grep "swirlix-v[0-9]" | sort -V | tail -n 1)
elif [ ! -z $SHOW_TAGS ] && [ $SHOW_TAGS != "true" ]; then
  # Attempt to get the requested Swirlix tag
  latest_swirlix_tag="${SHOW_TAGS}"
else
  # Get the latest Production Git tag for the "swirlix-" prefix
  latest_swirlix_tag=$(git tag --list | grep "swirlix-v[0-9]" | grep -v '\-testing' | sort -V | tail -n 1)
fi
...
download_latest_release() {
  ...
  if [ "$application" == "swirlix" ]; then
    local latest_tag_var="$latest_swirlix_tag"
  else
...
if [[ $SHOW_TAGS == "swirlix-"* ]] || [ -z $SHOW_TAGS ] ; then
  download_latest_release "swirlix"
fi
